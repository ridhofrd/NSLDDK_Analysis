...
public:
    ...
    void groupData(vector<double> &featureData)
    {
        int banyakKelas = 1 + static_cast<int>(3.3 * log10(featureData.size()));

        if (featureData.empty())
        {
            cout << "No data to group." << endl;
            return;
        }

        // Calculate statistics
        Statistics stats = calculateStatistics(featureData);
        cout << "Grouping data into " << banyakKelas << " classes..." << endl;

        // Group data into classes
        groupedData.clear();
        double range = stats.max - stats.min;
        double interval = range / banyakKelas;

        for (int i = 0; i < banyakKelas; i++)
        {
            GroupedData group;
            group.no = i + 1;
            group.startRange = stats.min + i * interval;
            group.endRange = group.startRange + interval;
            group.frequency = 0;

            for (double val : featureData)
            {
                if (val >= group.startRange && val < group.endRange)
                {
                    group.frequency++;
                }
            }
        ...
        cout << "No\tRange\t\t\tFrequency" << endl;
        cout << string(50, '-') << endl;
        for (const auto &group : groupedData)
        {
            cout << group.no << "\t[" << fixed << setprecision(2) << group.startRange << " - "
                 << group.endRange << ")\t" << group.frequency << endl;
        }
        cout << string(50, '-') << endl;
        cout << "Total Records: " << featureData.size() << endl;
        cout << "Number of Classes: " << banyakKelas << endl;
        cout << "Class Interval: " << fixed << setprecision(2) << interval << endl;
        cout << "Range: " << fixed << setprecision(2) << stats.min << " - " << stats.max << endl;
        cout << "Total Frequency: " << featureData.size() << endl;
    }

    // Fungsi untuk statistik deskriptif data berkelompok
    void groupDataStatistics()
    {
        if (groupedData.empty())
        {
            cout << "No grouped data available." << endl;
            return;
        }
        ...
        for (const auto &group : groupedData)
        {
            double midPoint = (group.startRange + group.endRange) / 2.0;
            weightedSum += midPoint * group.frequency;
        ...
        for (const auto &group : groupedData)
        {
            cumulativeFrequency += group.frequency;
            if (cumulativeFrequency >= totalFrequency / 2.0)
            {
                median = (group.startRange + group.endRange) / 2.0;
                break;
            }
        }
        cout << fixed << setprecision(4) << median << endl;
        cout << "Modus: ";
        int maxFrequency = 0;
        double mode = 0.0;
        for (const auto &group : groupedData)
        {
            if (group.frequency > maxFrequency)
            {
                maxFrequency = group.frequency;
                mode = (group.startRange + group.endRange) / 2.0;
            }
        }
        cout << fixed << setprecision(4) << mode << endl;
        cout << "Standard Deviation: ";
        double variance = 0.0;
        for (const auto &group : groupedData)
        {
            double midPoint = (group.startRange + group.endRange) / 2.0;
            variance += group.frequency * pow(midPoint - mean, 2);
        }
        variance /= totalFrequency;
        double stddev = sqrt(variance);
        cout << fixed << setprecision(4) << stddev << endl;
        cout << "Variance: " << fixed << setprecision(4) << variance << endl;
    }
    ...
    void CreateHistogram()
    {
        ...
        for (const auto &group : groupedData)
        {
            cout << group.no << "\t[" << fixed << setprecision(2) << group.startRange << " - "
                 << group.endRange << ")\t" << group.frequency << endl;
            // Histogram Bar
            int barLength = (group.frequency * 30) / groupedData.back().frequency; // Scale to 30 characters
            cout << string(barLength, '*') << endl;
        }
        cout << string(50, '-') << endl;
    }
    ...
    void analyze()
    {
        ...
        cout << "10 Atribute paling berpengaruh ke hasil" << endl;
        ...
        for (int i = 0; i < selectedIndices.size(); i++)
        {
            ...
            // Calculate statistics
            cout << "\n2. STATISTIK DESKRIPTIF DATA TUNGGAL" << endl;
            Statistics stats = calculateStatistics(featureData);
            ...
            // Data Berkelompok
            cout << "\n3 DATA BERKELOMPOK" << endl;
            groupData(featureData);

            cout << "\n4 STATISTIK DESKRIPTIF DATA BERKELOMPOK" << endl;
            groupDataStatistics();

            // Create histogram
            cout << "\n3. HISTOGRAM" << endl;
            createHistogram(featureData, banyakKelas);
            ...
            cout << "\n=== ANALISA ATRIBUT " << attrName << " SELESAI ===\n\n\n"
                 << endl;
        }
    }
...
int main()
{
    ...
    // Load data files
    analyzer.loadData("C:\\Users\\Muhammad Ridho F\\Desktop\\POLBAN\\Statistika Dan Probabilitas\\Jobsheet_5\\proogram-statpro\\proogram-statpro\\KDDTrain+.arff",
                      "C:\\Users\\Muhammad Ridho F\\Desktop\\POLBAN\\Statistika Dan Probabilitas\\Jobsheet_5\\proogram-statpro\\proogram-statpro\\KDDTest+.arff");
    // Perform analysis
    analyzer.analyze();
